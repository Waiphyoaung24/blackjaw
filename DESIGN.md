...
version: alpha
name: Blackjaw
description: A high-contrast monochrome consumer system anchored on a clean white canvas and a single warm yellow voltage (#f7bd02) carrying every primary CTA and brand mark. Structurally borrows Airbnb's modest-type / soft-shape / single-voltage discipline, but the palette is its own — white, ink (#282828), and yellow, with no secondary brand color. Because yellow has low contrast against white, CTA labels render in ink (#282828), never white. Type runs at modest weights — display 500/600 rather than the heavy 700+ that fintech and enterprise systems use; the brand trusts photography and generous whitespace over typographic muscle. Pill-shaped search bars (`{rounded.full}`), softly rounded cards (`{rounded.lg}` ~14px), and 32px button radii read as friendly and human — there is no hard corner anywhere except the body grid.

colors:
  primary: "#f7bd02"
  primary-active: "#d9a300"
  primary-disabled: "#fde79a"
...
- Single accent color: `{colors.primary}` (#f7bd02 — "Rausch") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. Used scarcely — most pages are 90% white + ink with one or two Rausch moments.
...
- **Rausch** (`{colors.primary}` — #f7bd02): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links. The most recognizable color in consumer travel.
- **Rausch Active** (`{colors.primary-active}` — #d9a300): The press / pointer-down variant — slightly more saturated. Used on `{component.button-primary-active}`.
- **Rausch Disabled** (`{colors.primary-disabled}` — #fde79a): A pale tint used on disabled CTAs.
...
- **Ink** (`{colors.ink}` — #282828): The dominant text color on light surfaces. Display headlines, body paragraphs, primary nav links, and most inline link text. Never pure black.
...
- **Star Rating** (`{colors.star-rating}` — #282828): The same ink token — Airbnb's star icon and "4.81" rating numbers all render in ink rather than a yellow/gold color, which is a deliberate brand choice (yellow stars feel cheap in travel context).
...
**`button-primary-disabled`** — Pale Rausch tint at #fde79a with white text. Cursor not-allowed.
